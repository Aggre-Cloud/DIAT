...
5. **検証** — 本文テキストが失われていないことを確認（カバレッジ < 80% の場合に
   中止 — 部分的な出力は許容されません）。
6. **エクスポート** — Excel ブック：`ID / 章 / 节 / 需求原文 / English / <選択した
   言語>`。
...
| 出力形式 | Excel ブック（ID / 章 / 节 / 需求原文 / English / <選択した言語>） |
...
├── README.md                   # このファイル — ユーザー向けドキュメント（英語）
├── README_zh.md                # ユーザー向けドキュメント（中国語）、完全ミラー
...
| B | 章 (Chapter) | 最上位の章番号 ＋ タイトル |
| C | 节 (Section) | サブ章番号 ＋ タイトル |
| D | 需求原文 (Source) | ソース言語の完全な文 |
